...
# ── 模拟盘/实盘候选池（自动更新、自动生长）──────────────
# True = 模拟盘每天扫描**本地缓存的全部 sh/sz**作为候选池（而非只抽 MAX_TRAIN_STOCKS 只）；
#        再叠加动态流动性过滤 → 候选池=“全（已缓存）市场里流动好的票”，每天自动刷新。
LIVE_SCAN_ALL_CACHED = True
# 每个交易日由 daily_paper.py 从全A股列表抓多少只“还没缓存”的新票进来，使缓存逐步覆盖全市场。
# 0 = 不生长。几周内可补全（全A股 sh/sz 约 3800 只）。
GROW_CACHE_PER_DAY   = 80

# ── 数据 ─────────────────────────────────────────────
START_DATE       = "20150101"
WINDOW_SIZE      = 30             # 时间窗口（天）— 增加上下文
# 横截面排名特征：walk-forward 实测「动量排名」会放大追涨、在 2018/2022 熊市更差，
# 故默认关闭（保留代码，将来可换更好的横截面特征——如均值回归/价值排名——再试）。
USE_CROSS_SECTIONAL = False
FEATURE_DIM      = 23 if USE_CROSS_SECTIONAL else 19   # 16技术 + 3市场 (+4横截面，开启时)
...
MAX_POSITION_RATIO = 0.20        # 单股最大仓位（持仓5只时 5×20%≈满仓）
MAX_HOLDINGS       = 5           # 最大持仓数量（按要求集中持仓5只）
                                 # ⚠️ 5只比10只更集中：波动/回撤更大，是收益与风险的取舍
...
# ── 大盘择时闸（空仓避熊，最关键的风控）────────────────
# 只做多的策略在熊市会不停买入下跌股、放大亏损。用沪深300趋势做总闸：
# 大盘在均线之上才允许持股；跌破均线则清仓避险，等趋势重新转好再进场。
# walk-forward 实测：不加这个，2018/2022 熊市会亏 40%+；加上能大幅削减熊市回撤。
USE_MARKET_FILTER  = True        # 是否启用大盘择时闸
MARKET_MA_DAYS     = 20          # 沪深300 收盘价的均线天数（跌破=空仓，站上=可持股）

# ── 动态可交易池（point-in-time，回测去未来函数）────────
# 「信号扫全市场，下单只在流动性好的票」。流动性按**截至当日**的近 N 日成交额中位数判定，
# 绝不偷看未来——避免用"今天才知道哪些票活跃"去交易历史。卖出不受此限（随时可平仓）。
LIQ_WINDOW_DAYS    = 60          # 用近 N 个交易日的成交额中位数衡量流动性
LIQ_MIN_AMOUNT_YI  = 2.0         # 中位成交额 ≥ 此值(亿元)才允许买入；0=关闭该过滤
